...
public class UnifiedLogManager {
    ...
    public static boolean restoreGroupToTimestamp(UUID groupUUID, LocalDateTime targetTime) {
        try (Connection conn = BetterStorage.BSPlugin.getDatabaseManager().getConnection()) {
            ...
            if (group == null) {
                // group_data が存在しなければ、group_deleted_backup から復元
                group = DataIO.loadGroupData(groupUUID);
                if (group == null) {
                    group = UnifiedLogManager.loadDeletedGroupBackup(groupUUID);
                    if (group == null) {
                        return false;
                    }
                }
    ...
    //ロールバック/削除管理セクション
    //==============================================================
    public static @Nullable GroupData loadDeletedGroupBackup(UUID groupUUID) {
        DatabaseManager db = BetterStorage.BSPlugin.getDatabaseManager();
        try (Connection conn = db.getConnection();
             PreparedStatement ps = conn.prepareStatement(
                     "SELECT json_data FROM group_deleted_backup WHERE group_uuid = ? ORDER BY timestamp DESC LIMIT 1")) {
            ps.setString(1, groupUUID.toString());
            try (ResultSet rs = ps.executeQuery()) {
                ...
                return backup.groupData;
            }
        } catch (Exception e) {
            Bukkit.getLogger().warning("[BetterStorage] グループバックアップ復元失敗: " + e.getMessage());
            return null;
        }
    }
